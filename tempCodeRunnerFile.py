...
def bot_loop(status_callback):
    ...
    current_state = "STEP_1_CAST"
    last_logged_state = None
    lost_frames = 0
    rounds_completed = 0

    cast_time = time.time()
    reward_wait_time = time.time()
    ...
    # Lock onto the window coordinates
    ...
    while config.IS_RUNNING:
        ...
        if current_state == "STEP_1_CAST":
            ...
            if not vision.check_hook_visible(frame):
                ...
                cast_time = time.time()
                current_state = "STEP_2_WAIT_BITE"
                time.sleep(2.0)
            else:
                current_state = "STEP_2_WAIT_BITE"
            continue
        ...
        if current_state == "STEP_2_WAIT_BITE":
            if vision.check_hook_visible(frame):
                status_callback("Bite Detected! Confirming [F]...", "#34C759")
                reliable_press("f", 0.1)
        ...
        if current_state == "STEP_3_MINIGAME":
            ...
            if target_x is not None and dash_x is not None and target_width > 0:
                ...
                # ADJUSTED: High-precision clamp thresholds to trap the bar completely inside
                active_dir = controller._active_direction
                if active_dir is None:
                    if error_ratio > 0.20:  # Direct correction early at 20%
                        controller.move_left()
                    elif error_ratio < -0.20:
                        controller.move_right()
                elif active_dir == "left":
                    if error_ratio <= 0.0:  # Run until perfectly centered
                        controller.stop_moving()
        ...
        if current_state == "STEP_4_REWARD":
            status_callback("Waiting for Reward Screen...", "#BF5AF2")
            if vision.check_reward_visible(frame):
                status_callback("Reward Screen Detected! [ESC]", "#BF5AF2")

                time.sleep(1.5)
                reliable_press("escape", 0.2)
                ...
                time.sleep(4.5)
                current_state = "STEP_1_CAST"
            elif time.time() - reward_wait_time > 6.0:
                logging.warning("\n[!] Reward timeout. Returning to Step 1.")
                current_state = "STEP_1_CAST"
            continue
